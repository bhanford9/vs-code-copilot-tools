...
> Before writing tests, load the `writing-csharp-tests` skill — it reads `LessonsLearned.GLOBAL.md` (cross-codebase patterns) and `LessonsLearned.md` (codebase-specific discoveries: enum disambiguation, non-mockable types, support helpers) from `~/Repos/vs-code-copilot-tools/skills/writing-csharp-tests/`
